                      ...
                      std::vector<cv::DMatch> &matches, std::string descriptorType, std::string matcherType, std::string selectorType)
{
    ...
    {    
        ...
        {
            descSource.convertTo(descSource, CV_32F);   //Binary Descriptors to floating point - OpenCV Error
...
void descKeypoints(vector<cv::KeyPoint> &keypoints, cv::Mat &img, cv::Mat &descriptors, string descriptorType)
{
    ...
    // KeyPoint Descriptors
        ...
        std::cout << "Keypoint indicators are invalid"<<std::endl;
...
void detKeypointsHarris(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis)
{
    ...
    // Prominent corners and instantiate keypoints
    ...
    for (size_t j = 0; j < dst_norm.rows; j++)
    {
        for (size_t i = 0; i < dst_norm.cols; i++)
        {
            ...
            { // only store points above a threshold
                ...
                {
                    double overlapping_keypoints = cv::KeyPoint::overlap(newKeyPoint, *it);
                    if (overlapping_keypoints > maxOverlap)
                    {
                        bOverlap = true;
